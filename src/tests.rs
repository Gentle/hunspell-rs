...
fn check() {
    ...
    assert_eq!(true, hs.check("cats"));
    assert_eq!(false, hs.check("nocats"));
}
...
fn check_with_added_word() {
    ...
    assert_eq!(true, hs.check("cats"));
    assert_eq!(false, hs.check("octonasaurius"));
    assert!(hs.add("octonasaurius"));
    assert_eq!(true, hs.check("octonasaurius"));
}
...
fn check_with_extra_dic() {
    let mut hs = Hunspell::new("tests/fixtures/reduced.aff", "tests/fixtures/reduced.dic");
    assert_eq!(true, hs.check("cats"));
    assert_eq!(false, hs.check("systemdunits"));
    assert!(hs.add_dictionary("tests/fixtures/extra.dic"));
    assert_eq!(true, hs.check("cats"));
    assert_eq!(true, hs.check("systemdunits"));
}
